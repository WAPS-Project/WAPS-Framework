...
namespace Waps\Framework\Session;

use Exception;
use Waps\Framework\Core\ErrorHandler;
use Waps\Framework\Support\Helper;
...
class SessionManager
{
	...
	{
		$myUsername = Application::checkRequest('post', 'username');
		$myPassword = Application::checkRequest('post', 'password');
	...
	public static function UserWelcome(): void
	{
		...
		$logoutCheck = Application::checkRequest('post', 'logout');
		if ($logoutCheck === 'TRUE') {
			session_destroy();
			header('Location: Home');
			exit;
	...
	{
		$username = filter_var(Application::checkRequest('post', 'username'), FILTER_SANITIZE_STRING);
		$firstName = filter_var(Application::checkRequest('post', 'firstName'), FILTER_SANITIZE_STRING);
		$secondName = filter_var(Application::checkRequest('post', 'lastName'), FILTER_SANITIZE_STRING);
		$email = filter_var(Application::checkRequest('post', 'email'), FILTER_SANITIZE_EMAIL);
		$age = Application::checkRequest('post', 'age');
		$password = filter_var(Application::checkRequest('post', 'pw'), FILTER_SANITIZE_STRING);
